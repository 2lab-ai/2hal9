...
use super::{
    ...
    contracts::{NeuronContract, IncentiveContract, ComputationMarketContract},
    incentives::{TokenEconomics, RewardDistribution, StakingMechanism},
    proof::{ProofVerifier, ComputationProof, ProofBuilder},
...
impl BlockchainService {
    pub async fn new(config: BlockchainConfig) -> Result<Self> {
        ...
        let client = Arc::new(
            BlockchainClient::new(config.chain.clone()).await
        ...
        let consensus = Arc::new(RwLock::new(
            ConsensusEngine::new(config.consensus.clone())
        ));
        
        // Create reward distribution
        let reward_distribution = Arc::new(RwLock::new(
            RewardDistribution::new(config.economics.clone())
        ));
    ...
    pub async fn register_neuron(
        ...
        // Register on-chain
        let tx_hash = self.neuron_contract
    ...
    pub async fn submit_task(
        ...
        let storage_proof = self.storage
            .store_with_redundancy(&signal_data, "signal")
            .await?;
        
        // Create task on market
        let task_id = self.market_contract
            .create_task(
...
impl BlockchainEventHandler {
    ...
    {
        self.event_listeners.write().await
