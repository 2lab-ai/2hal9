...
test_mode() {
    ...
    $HAL9_SERVER_BIN "$config_file" > "test-$mode.log" 2>&1 &
    SERVER_PID=$!
    ...
    $HAL9_CLI_BIN signal \
        ...
        --server localhost:$HAL9_PORT_MAIN
    
    # Check status
    echo ""
    echo "Checking status..."
    $HAL9_CLI_BIN status --server localhost:$HAL9_PORT_MAIN
...
test_mode "mock" "development" "$HAL9_CONFIG_DIR/config-3neurons.yaml"

# Test 2: Auto mode in development (should use mock)
test_mode "auto-dev" "development" "$HAL9_CONFIG_DIR/config-auto-mode.yaml"

# Test 3: Auto mode in production (should try API, fallback to mock if no key)
test_mode "auto-prod" "production" "$HAL9_CONFIG_DIR/config-auto-mode.yaml"

# Test 4: Hybrid mode (uses API if available and under limits)
test_mode "hybrid" "production" "$HAL9_CONFIG_DIR/config-hybrid-mode.yaml"
