...
if [ -z "$ANTHROPIC_API_KEY" ]; then
    log_warning "️  Warning: ANTHROPIC_API_KEY not set. API mode will fail and fallback to mock."
...
$HAL9_SERVER_CMD -- $HAL9_CONFIG_DIR/config-api-with-fallback.yaml &
SERVER_PID=$!
...
echo -e "\n📊 Initial metrics..."
curl -s http://localhost:$HAL9_PORT_MAIN/api/v1/metrics | jq '.data | {tokens_total, memory_usage_mb}'

# Submit a test signal
echo -e "\n🚦 Submitting test signal..."
RESPONSE=$(curl -s -X POST http://localhost:$HAL9_PORT_MAIN/api/v1/signal \
...
METRICS=$(curl -s http://localhost:$HAL9_PORT_MAIN/api/v1/metrics)
...
if [ -n "$ANTHROPIC_API_KEY" ]; then
    ...
    ANTHROPIC_API_KEY="invalid-key" $HAL9_SERVER_CMD -- $HAL9_CONFIG_DIR/config-api-with-fallback.yaml &
    SERVER_PID=$!
    sleep 5
    ...
    curl -s http://localhost:$HAL9_PORT_MAIN/api/v1/metrics | jq '.data | {signals_processed, errors_by_type}'
fi
