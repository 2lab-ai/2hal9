...
use crate::agent::{AssessmentQuestion, QuestionCategory, AgentLevel};
...
impl Default for AssessmentPool {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    fn add_questions(&mut self, questions: Vec<(&str, QuestionCategory, u8)>) {
        for (content, category, difficulty) in questions {
            ...
            let question = AssessmentQuestion {
                id: Uuid::new_v4(),
                category,
                difficulty: AgentLevel::from_value(difficulty).unwrap(),
...
impl Default for QuestionValidator {
    fn default() -> Self {
        Self::new()
    }
}
...
impl QuestionValidator {
    pub fn new() -> Self {
        ...
        required_keywords.insert(
            QuestionCategory::LogicalReasoning,
            ["if", "then", "therefore", "implies", "conclude"].iter().map(|s| s.to_string()).collect()
        );
        
        required_keywords.insert(
            QuestionCategory::SystemsThinking,
            ["system", "design", "architecture", "component", "interaction"].iter().map(|s| s.to_string()).collect()
        );
...
mod tests {
    ...
    fn test_assessment_pool() {
        ...
        for level in 1..=20 {
            let agent_level = AgentLevel::from_value(level).unwrap();
            let question = pool.get_question_for_level(agent_level);
            assert!(question.is_some());
        }
        
        // Test getting evaluation set
        let eval_set = pool.get_evaluation_set(AgentLevel::from_value(10).unwrap(), 6);
    ...
    fn test_question_validator() {
        ...
        let good_question = AssessmentQuestion {
            ...
            content: "If all philosophers are deep thinkers who contemplate the nature of existence, and some thinkers are brilliant mathematicians who work with abstract concepts, what logical conclusions can we derive about the relationship between philosophers and mathematicians in terms of their intellectual pursuits?".to_string(),
            time_limit: None,
        };
        ...
        let bad_question = AssessmentQuestion {
            ...
            difficulty: AgentLevel::from_value(15).unwrap(),
