...
impl MCPServer for NeuronMCPServer {
    ...
    async fn read_resource(&self, uri: &str) -> Result<Value> {
        match self.resources.get(uri) {
            ...
            None => Err(crate::Error::NotFound(format!("Resource '{}' not found", uri))),
        }
    }
    ...
    async fn handle_message(&self, message: MCPMessage) -> Result<MCPMessage> {
        match message {
                ...
                match req.method.as_str() {
                    ...
                    "tools/call" => {
                        ...
                        match self.call_tool(name, params).await {
                            ...
                            Err(e) => Ok(MCPMessage::error(req.id, error_codes::INTERNAL_ERROR, e.to_string())),
                        }
                    }
                    "resources/list" => {
                        Ok(MCPMessage::response(req.id, serde_json::json!({
                    ...
                    "resources/read" => {
                        let uri = req.params.get("uri")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| crate::Error::InvalidInput("Missing resource URI".to_string()))?;
                    ...
                    "neuron/processTask" => {
                        ...
                        match self.process_task(task_req).await {
                            Ok(result) => Ok(MCPMessage::response(req.id, serde_json::to_value(result)?)),
                            Err(e) => Ok(MCPMessage::error(req.id, error_codes::INTERNAL_ERROR, e.to_string())),
                        }
                    }
                    _ => {
            ...
            _ => {
                ...
                Ok(MCPMessage::error("0".to_string(), error_codes::INVALID_REQUEST, 
