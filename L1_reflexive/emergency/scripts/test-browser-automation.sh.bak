...
HTTP_PORT=8080 cargo run --bin hal9-server examples/browser-automation.yaml > browser-server.log 2>&1 &
SERVER_PID=$!
...
NAVIGATION_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/signal" \
...
EXTRACTION_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/signal" \
    -H "Content-Type: application/json" \
    -d '{
...
FORM_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/signal" \
    -H "Content-Type: application/json" \
    -d '{
...
BROWSER_METRICS=$(curl -s "http://localhost:8080/metrics" | grep "browser_")
