...
HTTP_PORT=8080 cargo run --bin hal9-server examples/codegen-neurons.yaml > codegen-server.log 2>&1 &
SERVER_PID=$!
...
HEALTH_RESPONSE=$(curl -s "http://localhost:8080/api/v1/codegen/health")
...
TEMPLATES_RESPONSE=$(curl -s "http://localhost:8080/api/v1/codegen/templates")
...
PROJECT_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/codegen/project" \
...
REVIEW_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/codegen/review" \
    -H "Content-Type: application/json" \
    -d '{
        "file_path": "test.py",
...
COMPLETION_RESPONSE=$(curl -s -X POST "http://localhost:8080/api/v1/codegen/complete" \
    -H "Content-Type: application/json" \
    -d '{
        "file_path": "main.rs",
        "cursor_position": 50,
...
if cargo run --bin hal9-codegen -- --help > /dev/null 2>&1; then
...
cargo run --bin hal9-codegen -- new --yes > cli-output.txt 2>&1 || true
