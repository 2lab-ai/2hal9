...
use crate::{
    api::WsMessage,
    claude::{ClaudeAPIClient, ClaudeInterface, FallbackClaude, HybridClaude, MockClaude},
    cost_tracker::CostTracker,
    error::{ServerError, ServerResult},
    metrics::Metrics,
    network::{ServiceDiscovery, TcpTransport},
    neuron::{ManagedNeuron, NeuronRegistry},
    router::{DistributedConfig, DistributedRouter, RoutingTable, SignalRouter},
};
use hal9_core::auth::{ApiKeyManager, JwtManager, UserManager};
use hal9_core::{neuron::NeuronHealth, Error, NeuronSignal, Result, ServerConfig};
...
impl HAL9Server {
    ...
    pub async fn initialize_auth(&mut self, pool: sqlx::SqlitePool) -> Result<()> {
        if self.config.auth.enabled {
            ...
            // Initialize tables
            user_manager.initialize().await.map_err(|e| {
                Error::Other(anyhow::anyhow!("Failed to initialize user tables: {}", e))
            })?;
            api_key_manager.initialize().await.map_err(|e| {
                Error::Other(anyhow::anyhow!(
                    "Failed to initialize API key tables: {}",
                    e
                ))
            })?;
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.start_time.write().await = Some(Instant::now());
        ...
        let memory_store = if self.config.memory.enabled {
            ...
            let memory_manager =
                crate::memory_manager::MemoryManager::new(&self.config.memory).await?;
        ...
        for neuron_config in &self.config.neurons {
            ...
            if self.config.backward_propagation.enabled {
                let base_prompt = neuron_config.system_prompt.clone().unwrap_or_else(|| {
                    format!(
                        "You are neuron {} on layer {}",
                        neuron_config.id, neuron_config.layer
                    )
                });
                neuron.enable_backward_propagation(
                    self.config.backward_propagation.clone(),
                    base_prompt,
                );
            }
        ...
        // Update metrics
        self.metrics
            .set_active_neurons(self.config.neurons.len() as u64);
        ...
        // Start signal router
        let mut router = SignalRouter::new(self.registry.clone(), self.routing_table.clone());
        router.start().await?;
        ...
        if self.config.network.enabled {
            if let (Some(transport), Some(discovery)) = (
                ...
                if let Some(ref _router) = *router_guard {
                    ...
                    let mut distributed_local_router =
                        SignalRouter::new(self.registry.clone(), self.routing_table.clone());
    ...
    async fn create_claude_instance(&self, layer: &str) -> Result<Box<dyn ClaudeInterface>> {
        match self.config.claude.mode.as_str() {
            ...
            "hybrid" | "auto" => {
                info!(
                    "Creating hybrid Claude for layer {} (mode: {})",
                    layer, self.config.claude.mode
                );
            ...
            loop {
                ...
                if !snapshot.errors_by_type.is_empty() {
                    for (error_type, count) in &snapshot.errors_by_type {
                        info!(
                            "[METRICS] {} - Error '{}': {} occurrences",
                            server_id, error_type, count
                        );
                    }
                }
            }
