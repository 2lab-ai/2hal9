//! HAL9 최소 로컬 데모 - 외부 의존성 없음

use std::collections::HashMap;

/// 간단한 로컬 뉴런
struct LocalNeuron {
    id: String,
    layer: String,
    memory: Vec<String>,
}

impl LocalNeuron {
    fn new(id: &str, layer: &str) -> Self {
        Self {
            id: id.to_string(),
            layer: layer.to_string(),
            memory: Vec::new(),
        }
    }

    fn process(&mut self, input: &str) -> String {
        // 로컬 처리 (외부 API 호출 없음)
        let response = match self.layer.as_str() {
            "L1" => format!("[반사] {}", input),
            "L2" => format!("[실행] {} 작업 수행", input),
            "L3" => format!("[계획] {} 전략 수립", input),
            _ => format!("[처리] {}", input),
        };

        // 메모리에 저장
        self.memory.push(format!("{}: {}", self.id, response.clone()));
        response
    }
}

/// 로컬 신경망
struct LocalNetwork {
    neurons: HashMap<String, LocalNeuron>,
}

impl LocalNetwork {
    fn new() -> Self {
        let mut neurons = HashMap::new();
        
        // 기본 뉴런 생성
        neurons.insert("n1".to_string(), LocalNeuron::new("n1", "L1"));
        neurons.insert("n2".to_string(), LocalNeuron::new("n2", "L2"));
        neurons.insert("n3".to_string(), LocalNeuron::new("n3", "L3"));

        Self { neurons }
    }

    fn send_signal(&mut self, neuron_id: &str, signal: &str) -> Result<String, String> {
        match self.neurons.get_mut(neuron_id) {
            Some(neuron) => Ok(neuron.process(signal)),
            None => Err(format!("뉴런 {} 없음", neuron_id)),
        }
    }
}

fn main() {
    println!("\n=== HAL9 로컬 전용 데모 ===");
    println!("외부 의존성: 없음 ✅");
    println!("인터넷 연결: 불필요 ✅");
    println!("빌드 시간: 최소 ✅");
    println!();

    // 로컬 네트워크 생성
    let mut network = LocalNetwork::new();

    // 개별 뉴런 테스트
    println!("1. 개별 뉴런 테스트:");
    
    match network.send_signal("n1", "안녕하세요") {
        Ok(response) => println!("   L1 뉴런: {}", response),
        Err(e) => println!("   오류: {}", e),
    }

    match network.send_signal("n2", "데이터 처리") {
        Ok(response) => println!("   L2 뉴런: {}", response),
        Err(e) => println!("   오류: {}", e),
    }

    match network.send_signal("n3", "프로젝트") {
        Ok(response) => println!("   L3 뉴런: {}", response),
        Err(e) => println!("   오류: {}", e),
    }

    println!();

    // 메모리 확인
    println!("2. 처리 히스토리:");
    if let Some(neuron) = network.neurons.get("n1") {
        for entry in &neuron.memory {
            println!("   {}", entry);
        }
    }

    println!();
    println!("✅ 모든 기능이 로컬에서만 실행되었습니다!");
    println!("✅ 외부 의존성 없이 정상 작동!");
}