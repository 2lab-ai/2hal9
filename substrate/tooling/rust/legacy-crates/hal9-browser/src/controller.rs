...
use crate::playwright_stub::{Browser, Playwright};
use std::sync::Arc;
...
impl BrowserController {
    ...
    pub async fn new(config: BrowserConfig) -> Result<Self> {
        ...
        // Create context pool
        let context_pool = Arc::new(Mutex::new(ContextPool::new(
            config.max_contexts,
            browser.clone(),
            config.clone(),
        )));
    ...
    pub async fn execute_action(&self, action: BrowserAction) -> Result<ActionResult> {
        ...
        let result = match &action {
            BrowserAction::Navigate { url } => self.navigate(context, url.clone()).await,
            BrowserAction::Click { selector } => self.click(context, selector.clone()).await,
            BrowserAction::Type { selector, text } => {
                self.type_text(context, selector.clone(), text.clone())
                    .await
            }
            BrowserAction::Extract {
                ...
                self.extract(context, selector.clone(), extract_type.clone())
                    .await
            }
            BrowserAction::Screenshot { full_page } => self.screenshot(context, *full_page).await,
            BrowserAction::WaitFor { condition } => self.wait_for(context, condition.clone()).await,
        };

        // Record completion metrics
        let duration = start.elapsed();
        self.metrics
            .record_action_complete(&action, &result, duration);
    ...
    async fn extract(
        ...
        let data = match extract_type {
            ExtractType::Text => page
                .text_content(&selector)
                .await
                .map_err(|e| BrowserError::Playwright(e.to_string()))?
                .unwrap_or_default(),
            ExtractType::Html => page
                .inner_html(&selector)
                .await
                .map_err(|e| BrowserError::Playwright(e.to_string()))?,
            ExtractType::Attribute(attr) => page
    ...
    async fn wait_for(
        ...
        match condition {
            WaitCondition::Selector(selector) => {
                page.wait_for_selector(&selector).await.map_err(|_| {
                    BrowserError::Timeout(format!("Waiting for selector: {}", selector))
                })?;
            }
