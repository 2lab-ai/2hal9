...
use governor::{clock::QuantaClock, Quota, RateLimiter};
...
use crate::{BrowserError, Result, SecurityConfig};
...
impl SecuritySandbox {
    ...
    pub fn new(config: SecurityConfig) -> Self {
        ...
        // Create URL policy
        let url_policy = UrlPolicy::new(config.url_whitelist.clone(), config.url_blacklist.clone());
    ...
    pub fn check(&self, url: &str) -> Result<()> {
        ...
        for pattern in &self.blacklist_patterns {
            if pattern.matches(url_str) {
                return Err(BrowserError::SecurityViolation(format!(
                    "URL matches blacklist: {}",
                    url
                )));
            }
        }
        ...
        if !self.whitelist_patterns.is_empty() {
            let whitelisted = self.whitelist_patterns.iter().any(|p| p.matches(url_str));
...
mod tests {
    ...
    fn test_url_policy() {
        ...
        assert!(policy
            .check("https://app.example.com/admin/settings")
            .is_err());
    ...
    fn test_credential_vault() {
        ...
        vault
            .store("example.com", "user".to_string(), "pass".to_string())
            .unwrap();
