...
pub fn create_api_router(server: Arc<HAL9Server>) -> Router {
    ...
    if server.jwt_manager.is_some() {
        ...
        let protected_auth_router = Router::new()
            ...
            .route(
                ...
                delete(api_auth::delete_api_key),
            )
    ...
    let codegen_router = Router::new()
        ...
        .route(
            ...
            get(api_codegen::list_templates),
        )
        .route(
            "/api/v1/codegen/project",
            post(api_codegen::generate_project),
        )
        .route(
            "/api/v1/codegen/project/:id",
            get(api_codegen::get_project_status),
        )
        .route(
            "/api/v1/codegen/complete",
            post(api_codegen::code_completion),
        )
...
async fn submit_signal(
    ...
    let signal = NeuronSignal::forward(
        "api-client",
        &req.neuron_id
            .unwrap_or_else(|| format!("neuron-{}", layer_str.to_lowercase())),
        "API",
        layer_str,
        req.content,
    );
...
async fn get_signal_trace(
    ...
    Ok(Json(ApiResponse::<SignalTrace>::error(
        "Signal tracing not yet implemented",
    )))
}
