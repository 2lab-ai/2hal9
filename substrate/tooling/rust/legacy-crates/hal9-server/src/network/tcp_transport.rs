...
use crate::network::protocol::{MessageCodec, NetworkMessage};
...
impl TcpTransport {
    ...
    async fn handle_connection(
        ...
        // Start read loop
        Self::connection_read_loop(connection, connections, signal_tx, config, metrics).await
    }
    ...
    async fn connection_read_loop(
        ...
        loop {
            ...
            // Read with timeout
            let read_result =
                timeout(config.io_timeout, async { stream.try_read(&mut buffer) }).await;
                ...
                Ok(Ok(n)) => {
                    ...
                    *connection.last_activity.write().await = std::time::Instant::now();
                    ...
                    match MessageCodec::decode(&buffer[..n]) {
                        Ok(msg) => {
                            if let Err(e) =
                                Self::handle_message(msg, &peer_id, &signal_tx, &metrics).await
                            {
    ...
    pub async fn connect(&self, address: SocketAddr, server_id: &str) -> Result<()> {
        ...
        tokio::spawn(async move {
            if let Err(e) =
                Self::connection_read_loop(connection, connections, signal_tx, config, metrics)
                    .await
            {
    ...
    pub async fn send_signal(&self, server_id: &str, signal: NeuronSignal) -> Result<()> {
        ...
        timeout(self.config.io_timeout, async { stream.try_write(&encoded) })
            .await
