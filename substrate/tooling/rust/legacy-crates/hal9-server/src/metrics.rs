...
impl Metrics {
    ...
    pub fn update_memory_usage(&self) {
        ...
        if let Ok(mem_info) = sys_info::mem_info() {
            ...
            self.memory_usage_bytes
                .store(used_kb * 1024, Ordering::Relaxed);
        }
    }
    ...
    pub fn snapshot(&self) -> MetricsSnapshot {
        ...
        for entry in self.errors_by_type.iter() {
            errors_by_type.insert(entry.key().clone(), entry.value().load(Ordering::Relaxed));
        }
        ...
        MetricsSnapshot {
            ...
            errors_by_type,
            memory_usage_mb: self.memory_usage_bytes.load(Ordering::Relaxed) as f64
                / (1024.0 * 1024.0),
        }
    }
