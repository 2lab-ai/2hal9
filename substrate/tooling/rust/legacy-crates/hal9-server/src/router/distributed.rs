...
use crate::{
    network::{
        discovery::{DiscoveryEvent, ServerInfo, ServiceDiscovery},
        TcpTransport,
    },
...
impl DistributedRouter {
    ...
    async fn handle_server_update(
        ...
        info!(
            "Discovered server {} with {} neurons",
            server.server_id,
            server.neurons.len()
        );
        ...
        for neuron in &server.neurons {
            ...
            debug!(
                "Mapped remote neuron {} to server {}",
                neuron.id, server.server_id
            );
        }
    ...
    /// Handle server loss
    async fn handle_server_lost(server_id: &str, remote_neurons: &Arc<DashMap<String, String>>) {
        ...
        if removed > 0 {
            debug!(
                "Removed {} remote neuron mappings for server {}",
                removed, server_id
            );
        }
    }
    ...
    async fn start_network_receiver(&mut self) -> Result<()> {
        ...
        tokio::spawn(async move {
            while let Some((from_server, signal)) = signal_rx.recv().await {
                ...
                signal
                    .metadata
                    .insert("from_server".to_string(), from_server);
                signal
                    .metadata
                    .insert("hop_count".to_string(), (hop_count + 1).to_string());
    ...
    pub async fn route_signal(&self, signal: NeuronSignal) -> Result<()> {
        ...
        if let Some(server_id) = self.remote_neurons.get(&signal.to_neuron) {
            ...
            debug!(
                "Routing signal {} to remote server {}",
                signal.signal_id, server_id
            );
            ...
            signal
                .metadata
                .insert("via_server".to_string(), self.server_id.clone());
    ...
    pub fn get_routing_info(&self) -> RoutingInfo {
        let remote_neurons: std::collections::HashMap<String, String> = self
            .remote_neurons
            .iter()
    ...
    pub async fn update_local_neurons(&self, neurons: Vec<String>) -> Result<()> {
        ...
        let neuron_infos: Vec<NeuronInfo> = neurons
            .into_iter()
            .map(|id| NeuronInfo {
                id: id.clone(),
