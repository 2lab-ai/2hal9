...
impl PluginLoader {
    ...
    pub async fn load_from_package(&self, package_path: &Path) -> Result<LoadedPlugin> {
        ...
        let file = std::fs::File::open(package_path)
            .context("Failed to open plugin package")?;
        ...
        // Extract other files
        self.extract_assets(&mut archive, &manifest.files, &install_path).await?;
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.is_dir() && path.join("manifest.json").exists() {
                match self.load_from_directory(&path).await {
                    Ok(plugin) => {
                        tracing::info!("Loaded development plugin: {} v{}", 
                            plugin.metadata.name, 
                            plugin.metadata.version
                        );
                        plugins.push(plugin);
    ...
    pub async fn deactivate_plugin(&self, plugin_id: &str) -> Result<()> {
        ...
        let _ = self.runtime.call_function(
            plugin_id,
            "on_deactivate",
            &[],
        ).await;
    ...
    fn extract_wasm(
        ...
        let mut wasm_file = archive.by_name(wasm_filename)
    ...
    async fn extract_assets(
        ...
        for example in &files.examples {
            if let Ok(mut file) = archive.by_name(example) {
                let filename = Path::new(example).file_name()
    ...
    fn verify_signature(&self, manifest: &PluginManifest, signature: &PluginSignature) -> Result<()> {
    ...
    fn default_security_policies() -> HashMap<String, SecurityPolicy> {
        ...
        policies.insert("neuron".to_string(), SecurityPolicy {
            allow_network: false,
            allow_filesystem: false,
            allow_system_time: true,
            allow_random: true,
        ...
        policies.insert("tool".to_string(), SecurityPolicy {
            allow_network: true,
