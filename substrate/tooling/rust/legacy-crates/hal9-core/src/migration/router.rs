...
impl MigrationRouter {
    ...
    pub fn route(&self, request: &RequestContext) -> RoutingDecision {
        ...
        if *self.shadow_mode.read()
            || self
                .feature_flags
                .is_feature_enabled("shadow_mode", request)
        {
...
impl RequestRouter {
    ...
    pub async fn process(&self, request: ProcessRequest) -> Result<ProcessResponse> {
        ...
        match decision {
            RoutingDecision::Flat => self.flat_handler.handle(request).await,
            RoutingDecision::Hierarchical => self.hierarchical_handler.handle(request).await,
