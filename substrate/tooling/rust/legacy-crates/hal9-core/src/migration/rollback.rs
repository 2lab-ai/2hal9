...
impl RollbackManager {
    ...
    async fn execute_immediate_rollback(&self, snapshot: &SystemSnapshot) -> Result<()> {
        ...
        self.restore_routing_config(&snapshot.routing_config)
            .await?;
    ...
    async fn execute_gradual_rollback(
        ...
        for step in 0..steps {
            ...
            self.update_traffic_percentage(hierarchical_percentage)
                .await?;
    ...
    async fn execute_partial_rollback(
        ...
        tracing::info!(
            "Executing partial rollback for components: {:?}",
            components
        );
        ...
        for component in components {
            match component.as_str() {
                ...
                "state" => {
                    self.restore_partial_state(&snapshot.system_state, component)
                        .await?;
                }
                _ => {
    ...
    async fn verify_system_health(&self) -> Result<()> {
        ...
        if !healthy {
            return Err(Error::Migration(
                "System health check failed after rollback".to_string(),
            ));
        }
        Ok(())
    }
