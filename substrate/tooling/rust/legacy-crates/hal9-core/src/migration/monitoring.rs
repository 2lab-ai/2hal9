...
impl MigrationMonitor {
    ...
    fn start_alert_monitoring(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                alerts
                    .write()
                    .retain(|a| chrono::Utc::now() - a.timestamp < chrono::Duration::minutes(5));
            }
        });
    }
    ...
    pub async fn is_stable_for(&self, duration: std::time::Duration) -> Result<bool> {
        ...
        let stable = history.recent(required_samples).all(|m| m.is_healthy());
...
impl HealthChecker {
    ...
    fn check(&self, metrics: &MigrationMetrics) -> HealthStatus {
        ...
        if metrics.hierarchical_error_rate > self.thresholds.max_error_rate {
            issues.push(format!(
                ...
                metrics.hierarchical_error_rate, self.thresholds.max_error_rate
            ));
        }
        ...
        if metrics.output_parity < self.thresholds.min_output_parity {
            issues.push(format!(
                ...
                metrics.output_parity, self.thresholds.min_output_parity
            ));
        }
    ...
    fn calculate_score(&self, metrics: &MigrationMetrics) -> f32 {
        ...
        if metrics.hierarchical_error_rate > self.thresholds.max_error_rate {
            score -= ((metrics.hierarchical_error_rate - self.thresholds.max_error_rate) / 0.05)
                .min(0.3);
        }
