...
use super::{
    ...
    negotiation::{
        DefaultNegotiator, NegotiationAgreement, NegotiationPreferences, NegotiationResponse,
        ProtocolDescriptor, ProtocolNegotiator, ProtocolOffer, SessionNegotiator,
    },
...
impl ProtocolManager {
    pub fn new(config: ProtocolManagerConfig, transport: Arc<DefaultTransport>) -> Self {
        ...
        let negotiator =
            SessionNegotiator::new(Box::new(default_negotiator), config.negotiation_timeout);
    ...
    pub async fn negotiate_with_peer(&self, peer_id: &str) -> Result<NegotiationAgreement> {
        ...
        let request_msg =
            serde_json::to_vec(&offer).map_err(|e| Error::Serialization(e.to_string()))?;

        self.transport
            .send(&format!("peer:{}:negotiation", peer_id), request_msg)
            .await?;
    ...
    pub async fn send_versioned_message(
        ...
        let versioned = VersionedMessage::new(protocol_id, protocol.version(), message.to_vec());

        // Serialize
        let data =
            serde_json::to_vec(&versioned).map_err(|e| Error::Serialization(e.to_string()))?;

        // Send
        self.transport
            .send(&format!("peer:{}:protocol:{}", peer_id, protocol_id), data)
            .await?;
    ...
    pub async fn receive_versioned_message(&self, data: &[u8]) -> Result<(String, Vec<u8>)> {
        ...
        let protocol = self.get_protocol(&versioned.protocol_id).ok_or_else(|| {
            Error::Protocol(format!("Unknown protocol: {}", versioned.protocol_id))
        })?;

        let message =
            if versioned.version != protocol.version() && self.config.version_migration_enabled {
    ...
    async fn receive_negotiation_response(&self, peer_id: &str) -> Result<NegotiationResponse> {
        let mut receiver = self
            .transport
            .receive::<Vec<u8>>(&format!("peer:{}:negotiation:response", peer_id))
            .await?;
