...
impl L5StrategicNeuron {
    ...
    fn generate_directives(&self, goals: &[Goal]) -> Vec<CognitiveOutput> {
        goals
            .iter()
            .flat_map(|goal| {
                let mut directives = vec![CognitiveOutput {
    ...
    async fn process(&mut self, input: Self::Input) -> Result<Self::Output> {
        ...
        let (content, metadata) = match analysis {
            StrategicAnalysis::VisionSetting => {
                ...
                let new_vision = self
                    .vision_model
                    .refine_vision(&self.state.read().current_vision, &input.content)?;
                ...
                let metadata: HashMap<String, serde_json::Value> = [
                    ...
                    (
                        "vision_length".to_string(),
                        serde_json::json!(new_vision.len()),
                    ),
                ]
                .into_iter()
                .collect();
            ...
            StrategicAnalysis::GoalManagement => {
                ...
                let metadata = [
                    ...
                    (
                        ...
                        serde_json::json!(goal_hierarchy.count_goals()),
                    ),
                ]
                .into_iter()
                .collect();
            ...
            StrategicAnalysis::PrincipleDefinition => {
                ...
                state
                    .strategic_context
                    .insert("principles".to_string(), serde_json::json!(principles));

                let metadata = [
                    (
                        "action".to_string(),
                        serde_json::json!("principle_definition"),
                    ),
                    (
                        "principle_count".to_string(),
                        serde_json::json!(principles.len()),
                    ),
                ]
                .into_iter()
                .collect();
            ...
            StrategicAnalysis::OpportunityAnalysis => {
                ...
                let metadata = [
                    (
                        "action".to_string(),
                        serde_json::json!("opportunity_analysis"),
                    ),
                    (
                        "opportunity_count".to_string(),
                        serde_json::json!(opportunities.len()),
                    ),
                ]
                .into_iter()
                .collect();
            ...
            StrategicAnalysis::General => {
                ...
                let metadata = [("action".to_string(), serde_json::json!("general_guidance"))]
                    .into_iter()
                    .collect();
        ...
        if gradient.error_signal.magnitude > 0.5 {
            ...
            for goal in &mut state.active_goals {
                if gradient
                    .error_signal
                    .context
                    .contains_key(&goal.id.to_string())
                {
