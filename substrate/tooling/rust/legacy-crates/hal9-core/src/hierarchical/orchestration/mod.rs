...
impl Orchestrator for DefaultOrchestrator {
    ...
    async fn add_unit(&mut self, unit: UnitDescriptor) -> Result<Uuid> {
        ...
        self.router
            .update_topology(routing::TopologyChange::NodeAdded {
                id: unit_id,
    ...
    async fn remove_unit(&mut self, unit_id: Uuid) -> Result<()> {
        ...
        // Update router
        self.router
            .update_topology(routing::TopologyChange::NodeRemoved { id: unit_id })
            .await?;
    ...
    async fn connect(&mut self, from: Uuid, to: Uuid, connection: Connection) -> Result<()> {
        ...
        self.topology_manager
            .add_edge(from, to, connection.clone())
            .await?;

        // Update router
        self.router
            .update_topology(routing::TopologyChange::LinkAdded {
                from,
                to,
    ...
    async fn disconnect(&mut self, from: Uuid, to: Uuid) -> Result<()> {
        ...
        // Update router
        self.router
            .update_topology(routing::TopologyChange::LinkRemoved { from, to })
            .await?;
    ...
    async fn route(&self, signal: OrchestrationSignal) -> Result<Vec<Uuid>> {
        ...
        let routable = RoutableSignal {
            ...
            signal_type: match signal.signal_type {
                ...
                SignalType::Control => routing::SignalType::Control {
                    command: "".to_string(),
                },
                SignalType::Data => routing::SignalType::Data {
    ...
    async fn optimize(&mut self) -> Result<OptimizationReport> {
        ...
        Ok(OptimizationReport {
            changes_made: vec![TopologyChange::ConnectionWeightChanged {
                from: Uuid::new_v4(),
                to: Uuid::new_v4(),
                old: 1.0,
                new: 0.8,
