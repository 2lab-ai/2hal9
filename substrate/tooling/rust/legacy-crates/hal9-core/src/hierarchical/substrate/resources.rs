    ...
    fn start_monitoring(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                allocations.retain(|_, alloc| alloc.expires_at.is_none_or(|exp| exp > now));
            }
        });
    }
            ...
            if let Some(max_cpu) = limits.max_cpu_cores {
                if request.cpu_cores.unwrap_or(0.0) > max_cpu {
                    return Err(Error::ResourceExhausted(format!(
                        ...
                        request.cpu_cores.unwrap_or(0.0),
                        max_cpu
                    )));
                }
            }
...
impl ComputeResource for LocalResources {
    async fn allocate(&self, request: ResourceRequest) -> Result<ResourceAllocation> {
        ...
        if cpu_allocated_cores + cpu_requested > cpu_available {
            return Err(Error::ResourceExhausted(format!(
                ...
                cpu_requested,
                cpu_available - cpu_allocated_cores
            )));
        }
        ...
        if memory_allocated_mb + memory_requested > memory_available_mb {
            return Err(Error::ResourceExhausted(format!(
                ...
                memory_requested,
                memory_available_mb - memory_allocated_mb
            )));
        }
        ...
        self.allocated_cpu
            .fetch_add((cpu_requested * 1000.0) as u64, Ordering::Relaxed);
        self.allocated_memory
            .fetch_add(memory_requested, Ordering::Relaxed);
    ...
    async fn release(&self, allocation: ResourceAllocation) -> Result<()> {
        if self.allocations.remove(&allocation.allocation_id).is_some() {
            ...
            self.allocated_cpu
                .fetch_sub((allocation.cpu_cores * 1000.0) as u64, Ordering::Relaxed);
            self.allocated_memory
                .fetch_sub(allocation.memory_mb, Ordering::Relaxed);
            Ok(())
        } else {
...
impl ComputeResource for ClusterResources {
    async fn allocate(&self, request: ResourceRequest) -> Result<ResourceAllocation> {
        let node_id = self
            .scheduler
            .select_node(&self.node_resources, &request)
        ...
        if !node.can_allocate(cpu_requested, memory_requested, gpu_requested) {
            return Err(Error::ResourceExhausted(
                "Node resources changed".to_string(),
            ));
        }
...
mod tests {
    ...
    async fn test_resource_limits() {
        ...
        resources
            .set_limits("limited-neuron", limits)
            .await
            .unwrap();
