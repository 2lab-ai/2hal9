...
mod tests {
    ...
    impl MetaLearner for MockMetaLearner {
        ...
        async fn optimize_architecture(&mut self) -> Result<ArchitectureUpdate> {
            Ok(ArchitectureUpdate {
                changes: vec![ArchitectureChange::AdjustParameters {
                    unit_id: Uuid::new_v4(),
                    params: {
                        let mut params = HashMap::new();
    ...
    async fn test_default_intelligence_coordinator() {
        let meta_learner = Box::new(MockMetaLearner {
            learning_efficiency: 0.5,
        });
        let self_organizer = Box::new(MockSelfOrganizer {
            organization_level: 0.3,
        });
        ...
        assert!(coordinator
            .enable_self_organization(self_org_config)
            .await
            .is_ok());
    }
