...
impl SelfOrganizingSystem {
        ...
        vec![
            ...
            OrganizationRule {
                ...
                condition: Box::new(|system| {
                    system.clusters.len() > 5 && system.hierarchy.is_none()
                }),
                action: Box::new(|system| {
            ...
            OrganizationRule {
                ...
                condition: Box::new(|system| {
                    let avg_connections = system
                        .units
                        .values()
                        .map(|u| u.connections.len())
                        .sum::<usize>() as f32
                        / system.units.len() as f32;
        ...
        for i in 0..cluster.members.len() {
            for j in (i + 1)..cluster.members.len() {
                if let (Some(unit_i), Some(unit_j)) = (
                    self.units.get(&cluster.members[i]),
                    self.units.get(&cluster.members[j]),
                ) {
    ...
    fn merge_clusters(&self, clusters: &[Cluster]) -> Vec<Cluster> {
        ...
        new_clusters.push(Cluster {
            ...
            purpose: format!(
                "merged_{}_{}",
                clusters[merge_pair.0].purpose, clusters[merge_pair.1].purpose
            ),
    ...
    fn cluster_distance(&self, cluster1: &Cluster, cluster2: &Cluster) -> f32 {
        ...
        for member1 in &cluster1.members {
            for member2 in &cluster2.members {
                if let (Some(unit1), Some(unit2)) =
                    (self.units.get(member1), self.units.get(member2))
                {
    ...
    async fn evolve_topology(&mut self) -> Result<TopologyUpdate> {
        ...
        for _ in 0..100 {
            ...
            if new_energy < best_energy
                || rand::random::<f32>() < ((best_energy - new_energy) / temperature).exp()
            {
