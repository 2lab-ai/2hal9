...
impl PromptAdjuster {
    ...
    pub fn apply_adjustment(&mut self, error: &ErrorType, adjustment: &Adjustment) -> Result<()> {
        ...
        let prompt_adjustment = PromptAdjustment {
            ...
            content: adjustment.suggested_value.as_str()
                .unwrap_or("")
                .to_string(),
            effectiveness: None,
        };
    ...
    fn determine_adjustment_type(&self, error: &ErrorType, adjustment: &Adjustment) -> Result<AdjustmentType> {
        match &adjustment.parameter[..] {
            "add_context" => Ok(AdjustmentType::PrependContext(
                adjustment.suggested_value.as_str().unwrap_or("").to_string()
            )),
    ...
    fn apply_adjustment_to_prompt(&self, prompt: &str, adjustment: &AdjustmentType) -> Result<String> {
        match adjustment {
            AdjustmentType::PrependContext(context) => {
                Ok(format!("{}\n\n{}", context, prompt))
            }
            AdjustmentType::AppendGuideline(guideline) => {
                Ok(format!("{}\n\nAdditional guideline: {}", prompt, guideline))
            }
            AdjustmentType::ReplaceSection { old, new } => {
                Ok(prompt.replace(old, new))
            }
    ...
    pub fn rollback_ineffective(&mut self, threshold: f32) {
        self.adjustments.retain(|adj| {
            adj.effectiveness.map(|e| e >= threshold).unwrap_or(true)
        });
        
        // Rebuild prompt from base + effective adjustments
        self.current_prompt = self.base_prompt.clone();
        for adj in &self.adjustments {
            if let Ok(new_prompt) = self.apply_adjustment_to_prompt(&self.current_prompt, &adj.adjustment_type) {
