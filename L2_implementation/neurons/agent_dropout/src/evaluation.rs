...
use crate::{
    agent::{AssessmentQuestion, AssessmentResponse, AssessmentScores, MutualEvaluation},
...
impl EvaluationEngine {
    ...
    pub fn evaluate_response(
        ...
        MutualEvaluation {
            ...
            confidence_level: self.calculate_confidence(evaluator_id, question.difficulty),
            comments: None,
            timestamp: Utc::now(),
        }
    }
    ...
    fn calculate_scores(
        ...
        AssessmentScores {
            logical_consistency: (base_score - time_penalty).max(0.0),
            creative_approach: base_score * 0.9,
            depth_of_thought: if response.reasoning.is_some() { base_score } else { base_score * 0.7 },
            clarity_efficiency: (1.0 - time_penalty).max(0.0),
        }
    }
    ...
    async fn aggregate_evaluations(
        ...
        if evaluations.is_empty() {
            return LevelEstimate {
                estimated_level: AgentLevel::new(5).unwrap(),
                confidence: 0.0,
                evaluator_count: 0,
                consensus_score: 0.0,
            };
        ...
        for eval in &evaluations {
            let score = eval.assessment_scores.weighted_average();
            let weight = eval.confidence_level;
        ...
        LevelEstimate {
            estimated_level: AgentLevel::new(final_level.clamp(1, 20)).unwrap(),
...
impl BayesianLevelEstimator {
    pub fn new() -> Self {
        ...
        for i in 0..20 {
            ...
            prior[i] *= 1.0 - (distance_from_center * 0.3);
        }
    ...
    pub fn estimate_level(
        ...
        for eval in evaluations {
            let score = eval.assessment_scores.weighted_average();
            let confidence = eval.confidence_level;
        ...
        LevelEstimate {
            estimated_level: AgentLevel::new(best_level).unwrap(),
            confidence: max_prob,
            evaluator_count: evaluations.len(),
...
mod tests {
    use super::*;
    use crate::agent::QuestionCategory;
    ...
    async fn test_evaluation_engine() {
        ...
        engine.register_agent(Uuid::new_v4(), AgentLevel::new(5).unwrap());
        engine.register_agent(Uuid::new_v4(), AgentLevel::new(10).unwrap());
        engine.register_agent(Uuid::new_v4(), AgentLevel::new(15).unwrap());
    ...
    fn test_bayesian_estimator() {
        ...
        for i in 0..5 {
            evaluations.push(MutualEvaluation {
                ...
                evaluatee_id: Uuid::new_v4(),
                assessment_scores: AssessmentScores {
                    logical_consistency: 0.7,
                    creative_approach: 0.8,
                    depth_of_thought: 0.75,
                    clarity_efficiency: 0.85,
                },
